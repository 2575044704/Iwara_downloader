...
class IwaraBatchDownloader:
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    ...
                    # 错误页面检测
                    print("[Playwright] 检查页面状态...")
                    # 等待页面加载
                    page.wait_for_timeout(2000)  # 增加到2秒确保页面加载完成
                    ...
                    # 方法1: 检查特定的错误div
                    try:
                        error_div = page.locator('div.text.text--h2.text--bold:has-text("Error")')
                        if error_div.count() > 0:
                            is_error_page = True
                            print("[Playwright] 检测到错误标题元素")
                    except:
                        pass
                    ...
                    if not is_error_page:
                        try:
                            ...
                            if '<div class="text text--h2 text--bold">错误</div>' in page_content:
                                is_error_page = True
                                print("[Playwright] 在页面内容中找到错误标记")
                        except:
                            pass
                    
                    # 如果检测到错误页面，直接返回
                    if is_error_page:
                        error_info = '页面显示错误（404或其他错误）'
                        print(f"[Playwright] {error_info} - 跳过此视频")
                        result['error'] = error_info
                        # 立即关闭资源并返回
                        page.close()
                        context.close()
                        browser.close()
                        self.last_playwright_error = error_info
                        return None
                    
                    print("[Playwright] 页面正常，等待API响应...")
                ...
                if result['data']:
                    ...
                    if result['error']:
                        ...
                        self.last_playwright_error = '未知错误'
                    
                    # 如果是特定错误且重试次数未超限，则重试
                    if retry_count < 2 and self.last_playwright_error and "Executable doesn't exist" in self.last_playwright_error:
    ...
    def download_video_aria2c(self, download_url, filename):
        ...
        return self.download_video_wget(download_url, filename)  # 直接调用 wget
    ...
    def process_video(self, video_id, json_filename, save_dir='downloads'):
        ...
        if not video_data:
            ...
            # 如果是页面错误，使用不同的记录方式
            if '页面显示错误' in error_reason:
                self.failed_downloads.append({
                    'id': video_id,
                    'json_file': json_filename,
                    'reason': '视频不存在或已删除（错误页面）',
                    'time': time.strftime('%Y-%m-%d %H:%M:%S')
                })
                print(f"[跳过] 视频页面不存在: {video_id}")
            else:
                self.failed_downloads.append({
                    'id': video_id,
                    'json_file': json_filename,
